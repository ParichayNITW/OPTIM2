...
def solve_pipeline(
    ...
    # --- SEGMENT FLOWS AFTER DELIVERY/SUPPLY ---
    segment_flows = [FLOW]
    for stn in stations:
    ...
    # --- IDENTIFY PUMP STATIONS & EXTRACT PUMP PARAMETERS ---
    pump_indices   = []
    diesel_pumps   = []
    electric_pumps = []
    Acoef = {}; Bcoef = {}; Ccoef = {}
    ...
    min_u = {
        j: max(1, (stations[j-1].get('MinRPM', 1000)+9)//10)
        for j in pump_indices
    }
    max_u = {
        j: max(min_u[j], stations[j-1].get('DOL', 1000)//10)
        for j in pump_indices
    }
    ...
    for i in range(1, N+1):
        ...
        if i in pump_indices:
            ...
            EFFP[i] = (
                model.Pcoef[i]*ratio**4 +
                model.Qcoef[i]*ratio**3 +
                model.Rcoef[i]*ratio**2 +
                model.Scoef[i]*ratio +
                model.Tcoef[i]
            ) / 100.0
        else:
    ...
    for i in range(1, N+1):
        ...
        for peak in peaks_dict[i]:
            ...
            expr = (
                model.RH[i]
                + (TDH[i]*model.NOP[i] if i in pump_indices else 0)
                - (peak['elev']-model.z[i])
                - loss0
            )
    ...
    dr_points = sorted({
        pt for df in DRA_CURVE_DATA.values() if df is not None for pt in df['%Drag Reduction'].values
    })
    ...
    # --- SOLVE ON NEOS WITH GLOBAL MINLP ---
