...
def solve_pipeline(stations, terminal, FLOW, KV, rho, RateDRA, Price_HSD):
    ...
    for i, stn in enumerate(stations, start=1):
        ...
        result[f"sdh_{name}"] = pyo.value(SH[i] + DH[i])
        if i in pump_indices:
            result[f"coef_A_{name}"] = stn['A']
            result[f"coef_B_{name}"] = stn['B']
            result[f"coef_C_{name}"] = stn['C']
            result[f"coef_P_{name}"] = stn.get('P', None)
            result[f"coef_Q_{name}"] = stn.get('Q', None)
            result[f"coef_R_{name}"] = stn.get('R', None)
            result[f"coef_S_{name}"] = stn.get('S', None)
            result[f"coef_T_{name}"] = stn.get('T', None)
    # Terminal station results (no pump)
    term_name = terminal['name'].strip()
