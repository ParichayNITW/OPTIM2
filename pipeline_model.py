...
def solve_pipeline(stations, terminal, RateDRA, Price_HSD):
    ...
    Each station includes: density (rho), viscosity (KV), and all geometry.
    """
    ...
    for i, stn in enumerate(stations, start=1):
        ...
        # Fluid per station
        rho[i] = stn.get('rho', 850.0)
        kv[i] = stn.get('kv', 10.0)
        # Pumps
    ...
    model.rho = pyo.Param(model.I, initialize=rho)
    model.kv = pyo.Param(model.I, initialize=kv)
    ...
    model.N_u = pyo.Var(model.pump_stations, domain=pyo.NonNegativeIntegers,
                       bounds=lambda m,j: (speed_min[j], speed_max[j]), initialize=lambda m,j: (speed_min[j]+speed_max[j])//2)
    ...
    for i in range(1, N+1):
        ...
        flow_m3s = stations[i-1].get('FLOW', 1000.0) / 3600.0
        v[i] = flow_m3s / area if area > 0 else 0.0
        kv_val = kv[i]
        if kv_val > 0:
            Re[i] = v[i]*d_inner[i]/(float(kv_val)*1e-6)
        else:
            Re[i] = 0.0
        if Re[i] > 0:
            if Re[i] < 4000:
    ...
    for i in range(1, N+1):
        ...
        if i in pump_indices:
            FLOW = stations[i-1].get('FLOW', 1000.0)
            DOL = model.DOL[i] if hasattr(model, 'DOL') else 1500
            TDH[i] = (model.A[i]*FLOW**2 + model.B[i]*FLOW + model.C[i]) * ((model.N[i]/DOL)**2)
            flow_eq = FLOW * DOL/model.N[i]
            EFFP[i] = (
    ...
    for i in range(1, N+1):
        ...
        MAOP_head = (2*thickness[i]*(smys[i]*0.070307)*design_factor[i]/D_out)*10000.0/rho[i]
        if i in pump_indices:
    ...
    for i in pump_indices:
        FLOW = stations[i-1].get('FLOW', 1000.0)
        rho_i = rho[i]
        eff_val = float(EFFP[i]) if hasattr(EFFP[i], "__float__") else 1
        power_kW = (rho_i * FLOW * 9.81 * TDH[i] * model.NOP[i])/(3600.0*1000.0*eff_val*0.95)
    ...
    for i, stn in enumerate(stations, start=1):
        ...
        if i in pump_indices and num_pumps>0:
            FLOW = stn.get('FLOW', 1000.0)
            rho_i = rho[i]
            eff_val = float(pyo.value(EFFP[i])) if hasattr(EFFP[i], "__float__") else 1
            power_kW = (rho_i * FLOW * 9.81 * float(pyo.value(TDH[i])) * num_pumps)/(3600.0*1000.0*eff_val*0.95)
            if i in electric_pumps:
        ...
        if i in pump_indices:
            dra_cost = (float(pyo.value(model.DR[i]))/4)*(stn.get('FLOW', 1000.0)*1000.0*24.0/1e6)*RateDRA
