...
def solve_pipeline(stations, terminal, FLOW, KV_list, rho_list, RateDRA, Price_HSD, *args, rpm_step_coarse=200, dra_step_coarse=10, rpm_step_fine=50, dra_step_fine=5):
    max_types = 2
    max_nop = 3
    N = len(stations)
    results_list = []

    def grid_search(stations, rpm_step, dra_step, neighborhoods=None):
        ...
        for idx, stn in enumerate(stations, start=1):
            if stn.get("is_pump", False):
                ...
                # Neighborhood grid (for zoom-in)
                if neighborhoods is not None and idx-1 < len(neighborhoods) and neighborhoods[idx-1] is not None:
                    r1_min, r1_max, r2_min, r2_max, d_min, d_max = neighborhoods[idx-1]
                    min_rpm1 = max(r1_min, int(pump_types[0].get("MinRPM", 1000)))
                    max_rpm1 = min(r1_max, int(pump_types[0].get("DOL", 3000)))
                    ...
                    if len(pump_types) > 1:
                        min_rpm2 = max(r2_min, int(pump_types[1].get("MinRPM", 1000)))
                        max_rpm2 = min(r2_max, int(pump_types[1].get("DOL", 3000)))
                    ...
                    dr_min = max(d_min, 0)
                    dr_max = min(d_max, max_dr)
                    drs = list(range(dr_min, dr_max+1, dra_step))
                    if drs and drs[-1] != dr_max:
                        drs.append(dr_max)
                else:
    ...
    coarse_results = grid_search(stations, rpm_step_coarse, dra_step_coarse)
    if not coarse_results:
        return {"error": True, "message": "No feasible solution found in coarse search. Please relax your constraints."}
    topN = sorted(coarse_results, key=lambda x: x['total_cost'])[:5]
    ...
    for top in topN:
        ...
        for idx, stn in enumerate(stations):
            if stn.get("is_pump", False):
                # Get RPMs from best config, allow ±100 RPM, DR ±5%
                st_cfg = top['station_config'][idx]
                rpm1 = st_cfg['RPM1']
                rpm2 = st_cfg['RPM2']
                dr = st_cfg['DR']
                r1_min = max(rpm1 - 100, int(stn['pumps'][0].get("MinRPM", 1000)))
                r1_max = min(rpm1 + 100, int(stn['pumps'][0].get("DOL", 3000)))
                if len(stn['pumps']) > 1:
                    r2_min = max(rpm2 - 100, int(stn['pumps'][1].get("MinRPM", 1000)))
                    r2_max = min(rpm2 + 100, int(stn['pumps'][1].get("DOL", 3000)))
                else:
                    r2_min = r2_max = 0
                d_min = max(dr - 5, 0)
                d_max = min(dr + 5, int(stn.get("max_dr", 30)))
                neighborhoods.append((r1_min, r1_max, r2_min, r2_max, d_min, d_max))
            else:
                neighborhoods.append(None)
        fine_results = grid_search(stations, rpm_step_fine, dra_step_fine, neighborhoods)
        fine_results_all.extend(fine_results)

    all_results = sorted(fine_results_all, key=lambda x: x['total_cost'])
    if not all_results:
        return {"error": True, "message": "No feasible solution found in fine search. Please check your input and relax constraints."}
    result = all_results[0]
    result['top3'] = all_results[:3]
    result['error'] = False
    return result
