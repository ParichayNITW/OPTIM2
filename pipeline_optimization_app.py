...
def _station_d_inner(stn: dict) -> float:
    """Return the internal pipe diameter (m) for a station dict."""
    if stn.get('D') is not None:
        t = float(stn.get('t', 0.007) or 0.007)
        return max(float(stn.get('D') or 0.0) - 2.0 * t, 0.0)
    d = stn.get('d_inner') or stn.get('d', 0.711)
    return float(d or 0.711)
...
def map_vol_linefill_to_segments(
    ...
    volume_from_km = pipeline_model._volume_from_km

    # Track remaining batch volume in m³ (volume is conserved; km depends on d_inner)
    seg_kv: list[float] = []
    seg_rho: list[float] = []
    ...
    i_batch = 0
    remaining_vol = batches[0]["volume_m3"]
    kv_cur = batches[0]["kv"]
    rho_cur = batches[0]["rho"]

    for stn, L in zip(stations, seg_lengths):
        d_seg = _station_d_inner(stn)
        if d_seg <= 0:
        ...
        seg_vol = volume_from_km(L, d_seg)
        need_vol = seg_vol
        ...
        while need_vol > 1e-9:
            if remaining_vol <= 1e-9:
                ...
                if i_batch >= len(batches):
                    length_km = km_from_volume(need_vol, d_seg)
                    segment_entries.append({"length_km": length_km, "kv": kv_cur, "rho": rho_cur})
                    need_vol = 0.0
                    break
                remaining_vol = batches[i_batch]["volume_m3"]
                kv_cur = batches[i_batch]["kv"]
                rho_cur = batches[i_batch]["rho"]
                if remaining_vol <= 1e-9:
                    continue

            take_vol = min(need_vol, remaining_vol)
            take_km = km_from_volume(take_vol, d_seg)
            if take_km > 0:
                segment_entries.append({"length_km": take_km, "kv": kv_cur, "rho": rho_cur})
            need_vol -= take_vol
        ...
        if L > 0:
            avg_rho = sum(e["length_km"] * e["rho"] for e in segment_entries) / L
        else:
...
def render_hgl_profile(stations: list, result: dict, terminal_name: str = "Terminal",
    ...
    if maop_vals:
        ...
        fig.add_hline(
            ...
            annotation_font=dict(color="#dc2626", size=12, family="Inter"),
    ...
    fig.add_hline(
        ...
        annotation_font=dict(color="#16a34a", size=12, family="Inter"),
    ...
    for i, stn in enumerate(stations):
        ...
        fig.add_annotation(
            ...
            showarrow=False,
            font=dict(size=12, color=col, family="Inter"),
            align="center",
    ...
    fig.add_annotation(
        ...
        showarrow=False,
        font=dict(size=12, color=t_col, family="Inter"),
        align="center",
        bgcolor="rgba(255,255,255,0.88)",
        bordercolor=t_col, borderwidth=1, borderpad=4,
        yanchor="top",
        yref="y",
    )
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title="Chainage (km)",
            title_font=dict(size=14, color="#374151", family="Inter"),
...
def _compute_variable_hourly_flows(
    ...
    """Block-based variable flow: 6 x 4-hour blocks, constant flow per block.

    Assigns higher flow to cheaper (lower tariff / lower viscosity) blocks and
    lower flow to expensive blocks, all within POR [q_min_run, q_max].
    Total across all active blocks equals total_volume_m3.
    Blocks:  07-11 | 11-15 | 15-19 | 19-23 | 23-03 | 03-07
    """
    import datetime as dt
    import numpy as np
    ...
    block_slices = [(i * BLOCK_SIZE, min((i + 1) * BLOCK_SIZE, n)) for i in range(n_blocks)]

    kv_ref = float(np.median(kv_per_hour)) if kv_per_hour else 10.0
    block_cost: list[float] = []
    for bs, be in block_slices:
        r_avg  = float(np.mean(rate_per_hour[bs:be])) if bs < be else default_rate
        kv_avg = float(np.mean(kv_per_hour[bs:be])) if bs < be else kv_ref
        block_cost.append(max(r_avg * max(kv_avg / kv_ref, 1e-6), 1e-9))

    # ── Assign block flows: cheapest → q_max, most expensive → q_min_run ───
    c_min, c_max = min(block_cost), max(block_cost)
    if c_max > c_min * 1.02:
        block_flow = [
            q_min_run + (q_max - q_min_run) * (c_max - c) / (c_max - c_min)
            for c in block_cost
        ]
    else:
        # Flat tariff / single product: uniform distribution
        block_flow = [total_volume_m3 / (n_blocks * BLOCK_SIZE)] * n_blocks

    # ── Volume balance: scale + clamp iteratively ───────────────────────────
    for _ in range(30):
        total_fl = sum((be - bs) * f for f, (bs, be) in zip(block_flow, block_slices))
        if abs(total_fl - total_volume_m3) < 0.5:
            break
        scale = total_volume_m3 / max(total_fl, 1.0)
        block_flow = [max(q_min_run, min(q_max, f * scale)) for f in block_flow]

    # ── Optional shutdown: drop most expensive blocks if volume already met ─
    sorted_by_cost = sorted(range(n_blocks), key=lambda i: block_cost[i], reverse=True)
    active_vol = sum((be - bs) * block_flow[bi] for bi, (bs, be) in enumerate(block_slices))
    for bi in sorted_by_cost:
        bs, be = block_slices[bi]
        removed = (be - bs) * block_flow[bi]
        if block_flow[bi] > 0 and active_vol - removed >= total_volume_m3 - 1.0:
            active_vol -= removed
            block_flow[bi] = 0.0

    # ── Expand to per-hour list ─────────────────────────────────────────────
    result: list[float] = []
    for bi, (bs, be) in enumerate(block_slices):
        for _ in range(be - bs):
            result.append(round(max(0.0, block_flow[bi]), 1))
    while len(result) < n:
        result.append(round(nominal_flow, 1))
    return result[:n]
...
if not auto_batch:
    ...
    if run_day or run_hour:
        ...
        if (
            ...
            for _qt in hourly_flow_rates_arg:
                if _qt <= 0:
                    # Shutdown block: FLOW_sched keeps at least one feasible candidate
                    _cands = [round(FLOW_sched, 1)]
                elif _qt > FLOW_sched * 1.005:
                    # High-flow (cheap) block: provide ONLY the target so the per-hour
                    # solver cannot revert to FLOW_sched (which would be cheaper per-hour).
                    # Q_b > FLOW_sched ≥ hydraulic minimum → always feasible.
                    _cands = [round(min(_q_hi, _qt), 1)]
                elif _qt < FLOW_sched * 0.995:
                    # Low-flow (expensive) block: prefer lower target; FLOW_sched as
                    # fallback if hydraulics prevent running below it.
                    _cands = sorted({round(_qt, 1), round(FLOW_sched, 1)})
                else:
                    _cands = [round(FLOW_sched, 1)]
    ...
    if st.session_state.get("run_mode") in ("hourly", "daily") and st.session_state.get("day_df") is not None:
        ...
        with tab_summary:
            ...
            # ── HGL profile — user-selected hour ─────────────────────────────
            if reports:
                _hgl_hour_options = [f"{rec['time']:02d}:00" for rec in reports]
                _hgl_sel = st.selectbox(
                    "Select hour for Hydraulic Grade Line",
                    options=_hgl_hour_options,
                    index=len(_hgl_hour_options) - 1,
                    key="hgl_hour_select",
                )
                _hgl_rec = reports[_hgl_hour_options.index(_hgl_sel)]
                _last_res_hgl = _hgl_rec["result"]
                _term_elev_hgl = float(term_data.get("elev", 0.0) or 0.0)
                _term_head_hgl = float(term_data.get("min_residual", 50.0) or 50.0)
                _term_name_hgl = str(term_data.get("name", "Terminal"))
                _hgl_fig_summary = render_hgl_profile(
                    stations_base, _last_res_hgl,
                    terminal_name=_term_name_hgl,
                    terminal_elev=_term_elev_hgl,
                    terminal_head=_term_head_hgl,
                )
                if _hgl_fig_summary is not None:
                    st.subheader(f"Pipeline Hydraulic Grade Line — {_hgl_sel}")
                    st.plotly_chart(
                        _hgl_fig_summary,
                        use_container_width=True,
                        config={"displayModeBar": True, "scrollZoom": True, "displaylogo": False},
                        key="hgl_profile_summary",
                    )
                ...
                if not _pump_stns:
                    ...
                    for _pi, (_pt, _ps) in enumerate(zip(_ps_sub_tabs, _pump_stns)):
                        with _pt:
                            ...
                            # Check for multi-type pump configuration
                            ...
                            def _draw_pump_curves(
                                fig_title, eff_title,
                                cA, cB, cC, cP, cQc, cRe, cSe, cTe,
                                n_pumps, dol, min_rpm, act_rpm,
                                act_flow, act_head, act_eff, drag_red,
                                type_label=""
                            ):
                                # Find shutoff flow: where DOL curve H=0 (cA*q^2+cB*q+cC=0)
                                _q_shutoff = None
                                if cA < 0 and cA != 0:
                                    _disc = cB**2 - 4 * cA * cC
                                    if _disc >= 0:
                                        _q_shutoff = (-cB - math.sqrt(_disc)) / (2 * cA)
                                        if _q_shutoff <= 0:
                                            _q_shutoff = None

                                # X-axis: clip at ~5% beyond operating point, never extrapolate far
                                if act_flow > 0:
                                    _q_lim = act_flow * 1.10
                                elif _q_shutoff is not None:
                                    _q_lim = _q_shutoff * 0.95
                                else:
                                    _q_lim = 2000.0
                                if _q_shutoff is not None:
                                    _q_lim = min(_q_lim, _q_shutoff * 0.98)
                                _Q = np.linspace(1.0, max(_q_lim, 10.0), 300)

                                def _h(_q, _r):
                                    _qe = _q / _r if _r > 0 else _q
                                    return n_pumps * np.maximum(cA * _qe**2 + cB * _qe + cC, 0.0) * _r**2

                                def _e(_q, _r):
                                    _qe = _q / _r if _r > 0 else _q
                                    return np.clip(cP*_qe**4 + cQc*_qe**3 + cRe*_qe**2 + cSe*_qe + cTe, 0, 100)

                                # Build list of RPM levels: every 50 RPM from DOL down to min_rpm
                                _dol_int = int(round(dol))
                                _min_int = int(round(min_rpm))
                                _act_int = int(round(act_rpm))
                                _step_start = int(round(dol / 50) * 50)
                                _step_end = int(round(min_rpm / 50) * 50)
                                _rpm_levels = list(range(_step_start, _step_end - 1, -50))
                                _rpm_set = sorted(set(_rpm_levels + [_dol_int, _min_int, _act_int]), reverse=True)

                                # Color gradient: deep blue (DOL) → orange (min RPM)
                                _n_curves = max(len(_rpm_set), 2)
                                _blue_rgb = (31, 119, 180)
                                _orange_rgb = (255, 127, 14)
                                def _blend(t):
                                    r = int(_blue_rgb[0] + t * (_orange_rgb[0] - _blue_rgb[0]))
                                    g = int(_blue_rgb[1] + t * (_orange_rgb[1] - _blue_rgb[1]))
                                    b = int(_blue_rgb[2] + t * (_orange_rgb[2] - _blue_rgb[2]))
                                    return f"rgb({r},{g},{b})"
                                _curve_colors = [
                                    _blend(i / (_n_curves - 1)) for i in range(_n_curves)
                                ]

                                _R_sys = act_head / (act_flow ** 2) if act_flow > 0 else 0.0
                                _H_sys = _R_sys * _Q ** 2
                                _R_nd  = _R_sys / max(1.0 - drag_red / 100.0, 0.01) if drag_red > 1.0 else _R_sys
                                _H_nd  = _R_nd * _Q ** 2

                                _col_hq, _col_ef = st.columns(2)
                                with _col_hq:
                                    _fhq = go.Figure()
                                    for _irpm, _rpm_val in enumerate(_rpm_set):
                                        _r = _rpm_val / dol if dol > 0 else 1.0
                                        _H_rpm = _h(_Q, _r)
                                        _is_act = abs(_rpm_val - _act_int) <= 1
                                        _is_dol = abs(_rpm_val - _dol_int) <= 1
                                        _is_min = abs(_rpm_val - _min_int) <= 1
                                        if _is_act:
                                            _dash = "solid"
                                            _width = 2.8
                                            _opacity = 1.0
                                            _label = f"{_rpm_val} RPM  ← actual"
                                        elif _is_dol:
                                            _dash = "dash"
                                            _width = 2.2
                                            _opacity = 0.9
                                            _label = f"{_rpm_val} RPM (DOL)"
                                        elif _is_min:
                                            _dash = "dot"
                                            _width = 2.0
                                            _opacity = 0.9
                                            _label = f"{_rpm_val} RPM (min)"
                                        else:
                                            _dash = "solid"
                                            _width = 1.0
                                            _opacity = 0.5
                                            _label = f"{_rpm_val} RPM"
                                        _fhq.add_trace(go.Scatter(
                                            x=_Q, y=_H_rpm,
                                            name=_label,
                                            line=dict(dash=_dash, color=_curve_colors[_irpm], width=_width),
                                            opacity=_opacity,
                                        ))
                                    if _R_sys > 0:
                                        _sys_lbl = "System curve (with DRA)" if drag_red > 1.0 else "System curve"
                                        _fhq.add_trace(go.Scatter(
                                            x=_Q, y=_H_sys, name=_sys_lbl,
                                    ...
                                    if act_flow > 0 and act_head > 0:
                                        _fhq.add_trace(go.Scatter(
                                            x=[act_flow], y=[act_head],
                                            mode="markers",
                                            marker=dict(symbol="circle", size=16, color="red",
                                                        line=dict(color="darkred", width=2)),
                                            name=f"Operating Point: {act_flow:.0f} m³/hr @ {act_head:.1f} m",
                                        ))
                                    _fhq.update_layout(
                                        title=dict(text=fig_title, font=dict(size=14)),
                                ...
                                with _col_ef:
                                    _fef = go.Figure()
                                    for _irpm2, _rpm_val2 in enumerate(_rpm_set):
                                        _r2 = _rpm_val2 / dol if dol > 0 else 1.0
                                        _Eff2 = _e(_Q, _r2)
                                        _is_act2 = abs(_rpm_val2 - _act_int) <= 1
                                        _is_dol2 = abs(_rpm_val2 - _dol_int) <= 1
                                        _is_min2 = abs(_rpm_val2 - _min_int) <= 1
                                        if _is_act2:
                                            _edash = "solid"; _ewidth = 2.8; _eopac = 1.0
                                            _elabel = f"η @ {_rpm_val2} RPM  ← actual"
                                        elif _is_dol2:
                                            _edash = "dash"; _ewidth = 2.0; _eopac = 0.9
                                            _elabel = f"η @ {_rpm_val2} RPM (DOL)"
                                        elif _is_min2:
                                            _edash = "dot"; _ewidth = 2.0; _eopac = 0.9
                                            _elabel = f"η @ {_rpm_val2} RPM (min)"
                                        else:
                                            _edash = "solid"; _ewidth = 1.0; _eopac = 0.45
                                            _elabel = f"η @ {_rpm_val2} RPM"
                                        _fef.add_trace(go.Scatter(
                                            x=_Q, y=_Eff2,
                                            name=_elabel,
                                            line=dict(dash=_edash, color=_curve_colors[_irpm2], width=_ewidth),
                                            opacity=_eopac,
                                        ))
                                    if act_flow > 0 and act_eff > 0:
                                        _fef.add_trace(go.Scatter(
                                            x=[act_flow], y=[act_eff],
                                            mode="markers",
                                            marker=dict(symbol="circle", size=16, color="red",
                                                        line=dict(color="darkred", width=2)),
                                            name=f"Operating Point: {act_eff:.1f}%",
                                        ))
                                    _fef.update_layout(
                                        title=dict(text=eff_title, font=dict(size=14)),
                                        xaxis_title="Flow (m³/hr)",
                            ...
                            if len(_valid_types) >= 2:
                                # Multi-type station: one sub-tab per type + Combined
                                _type_tab_labels = [f"Type {k}" for k in sorted(_valid_types.keys())] + ["Combined"]
                                _type_tabs = st.tabs(_type_tab_labels)
                                _sorted_type_keys = sorted(_valid_types.keys())
                                for _ti, (_tt, _tk) in enumerate(zip(_type_tabs[:-1], _sorted_type_keys)):
                                    with _tt:
                                        _tv = _valid_types[_tk]
                                        ...
                                        _t_dol     = float(_tv.get("DOL", _pdol) or _pdol)
                                        _t_min_rpm = float(_tv.get("MinRPM", _pmin_rpm) or _pmin_rpm)
                                        _t_avail   = int(_tv.get("available", 1) or 1)
                                        _t_act_rpm = float(
                                            _last_res.get(f"speed_{_pk}_{_tk}",
                                            _last_res.get(f"speed_{_pk}", _t_dol)) or _t_dol
                                        )
                                        _t_act_flow = float(
                                            _last_res.get(f"pump_flow_{_pk}_{_tk}",
                                        ...
                                        _t_act_eff = float(
                                            ...
                                            _last_res.get(f"efficiency_{_pk}", 0)) or 0
                                        )
                                        if _tC == 0 and _tA == 0 and _tB == 0:
                                            st.info(f"No head curve coefficients for Type {_tk} at {_pname}.")
                                            continue
                                        _draw_pump_curves(
                                            fig_title=f"{_pname} — Type {_tk}: H-Q & System Curves ({_t_avail} pump{'s' if _t_avail > 1 else ''})",
                                            eff_title=f"{_pname} — Type {_tk}: Efficiency vs Flow",
                                            cA=_tA, cB=_tB, cC=_tC,
                                            cP=_tP, cQc=_tQc, cRe=_tRe, cSe=_tSe, cTe=_tTe,
                                            n_pumps=_t_avail, dol=_t_dol, min_rpm=_t_min_rpm,
                                            act_rpm=_t_act_rpm, act_flow=_t_act_flow,
                                            act_head=_t_act_head, act_eff=_t_act_eff,
                                            drag_red=_pdr, type_label=_tk,
                                        )

                                # Combined tab: overlay all types on same chart
                                with _type_tabs[-1]:
                                    _q_comb = max(_pact_flow * 2.5, 500.0) if _pact_flow > 0 else 2000.0
                                    _Q_comb = np.linspace(1.0, _q_comb, 300)
                                    _fcomb = go.Figure()
                                    _colors_comb = ["royalblue", "seagreen", "darkorange", "purple", "firebrick"]
                                    for _ci, _ck in enumerate(_sorted_type_keys):
                                        _cv = _valid_types[_ck]
                                        _cA = float(_cv.get("A", 0) or 0)
                                        _cB = float(_cv.get("B", 0) or 0)
                                        _cC = float(_cv.get("C", 0) or 0)
                                        _c_dol = float(_cv.get("DOL", _pdol) or _pdol)
                                        _c_avail = int(_cv.get("available", 1) or 1)
                                        _c_act_rpm = float(
                                            _last_res.get(f"speed_{_pk}_{_ck}",
                                            _last_res.get(f"speed_{_pk}", _c_dol)) or _c_dol
                                        )
                                        _c_r = _c_act_rpm / _c_dol if _c_dol > 0 else 1.0
                                        _c_clr = _colors_comb[_ci % len(_colors_comb)]
                                        _H_c_dol = _c_avail * np.maximum(_cA * (_Q_comb)**2 + _cB * _Q_comb + _cC, 0.0)
                                        _H_c_act = _c_avail * np.maximum(_cA * (_Q_comb / _c_r)**2 + _cB * (_Q_comb / _c_r) + _cC, 0.0) * _c_r**2
                                        _fcomb.add_trace(go.Scatter(
                                            x=_Q_comb, y=_H_c_dol,
                                            name=f"Type {_ck} DOL",
                                            line=dict(dash="dash", color=_c_clr, width=2),
                                        ))
                                        _fcomb.add_trace(go.Scatter(
                                            x=_Q_comb, y=_H_c_act,
                                            name=f"Type {_ck} @ {_c_act_rpm:.0f} RPM",
                                            line=dict(color=_c_clr, width=2.5),
                                        ))
                                    # System curve
                                    if _pact_flow > 0 and _pact_head > 0:
                                        _R_sys_c = _pact_head / _pact_flow**2
                                        _fcomb.add_trace(go.Scatter(
                                            x=_Q_comb, y=_R_sys_c * _Q_comb**2,
                                            name="System curve", line=dict(color="firebrick", width=2),
                                        ))
                                        _fcomb.add_trace(go.Scatter(
                                            ...
                                            marker=dict(symbol="circle", size=16, color="red",
                                                        line=dict(color="darkred", width=2)),
                                            name=f"Operating Point: {_pact_flow:.0f} m³/hr @ {_pact_head:.1f} m",
                                        ))
                                    _fcomb.update_layout(
                                        title=dict(text=f"{_pname}: Combined Pump Types — H-Q Curves", font=dict(size=14)),
                                        xaxis_title="Flow (m³/hr)",
                                        yaxis_title="Head (m)",
                                    ...
                                    st.plotly_chart(_fcomb, use_container_width=True)

                            else:
                                # Single pump type (or no pump_types dict): use station-level coefficients
                                ...
                                # Use type A/B coefficients if they exist and station-level is empty
                                if _pC == 0 and _pA == 0 and _pB == 0 and len(_valid_types) == 1:
                                    _only_v = next(iter(_valid_types.values()))
                                    _pA  = float(_only_v.get("A", 0) or 0)
                                    _pB  = float(_only_v.get("B", 0) or 0)
                                    _pC  = float(_only_v.get("C", 0) or 0)
                                    _pP  = float(_only_v.get("P", 0) or 0)
                                    _pQc = float(_only_v.get("Q", 0) or 0)
                                    _pRe = float(_only_v.get("R", 0) or 0)
                                    _pSe = float(_only_v.get("S", 0) or 0)
                                    _pTe = float(_only_v.get("T", 0) or 0)
                                if _pC <= 0 and _pA == 0 and _pB == 0:
                                    st.info(f"No pump head curve coefficients configured for {_pname}.")
                                    continue
                                _draw_pump_curves(
                                    fig_title=f"{_pname}: H-Q & System Curves ({_pn} pump{'s' if _pn > 1 else ''} in series)",
                                    eff_title=f"{_pname}: Efficiency vs Flow",
                                    cA=_pA, cB=_pB, cC=_pC,
                                    cP=_pP, cQc=_pQc, cRe=_pRe, cSe=_pSe, cTe=_pTe,
                                    n_pumps=_pn, dol=_pdol, min_rpm=_pmin_rpm,
                                    act_rpm=_pact_rpm, act_flow=_pact_flow,
                                    act_head=_pact_head, act_eff=_pact_eff,
                                    drag_red=_pdr,
                                )
        ...
        with tab_3d:
            if not reports:
                ...
                # ── 3D Pump Efficiency surface: station × hour ───────────────
                _eff_z = []
                for _r in reports:
                    _row = []
                    for _s in stations_base:
                        _k = str(_s.get("name", "")).strip().lower().replace(" ", "_")
                        _row.append(float(_r["result"].get(f"pump_eff_{_k}", 0) or 0))
                    _eff_z.append(_row)
                if _eff_z and any(any(v > 0 for v in row) for row in _eff_z):
                    _fig_3d_eff = go.Figure(data=[go.Surface(
                        z=_eff_z,
                        x=_stn_names,
                        y=_hr_labels,
                        colorscale="RdYlGn",
                    ...
                    _fig_3d_eff.update_layout(
                        title="3D Pump Efficiency (%) — Station × Hour",
                        scene=dict(
                            xaxis_title="Station",
                            yaxis_title="Hour",
                    ...
                    st.plotly_chart(_fig_3d_eff, use_container_width=True)

                # ── 3D Residual Head surface: station × hour ─────────────────
                _rh_z = []
                for _r in reports:
                    _row = []
                    for _s in stations_base:
                        _k = str(_s.get("name", "")).strip().lower().replace(" ", "_")
                        _row.append(float(_r["result"].get(f"residual_head_{_k}", 0) or 0))
                    _rh_z.append(_row)
                if _rh_z and any(any(v > 0 for v in row) for row in _rh_z):
                    _fig_3d_rh = go.Figure(data=[go.Surface(
                        z=_rh_z,
                        x=_stn_names,
                        y=_hr_labels,
                        colorscale="Plasma",
                    )])
                    _fig_3d_rh.update_layout(
                        title="3D Residual Head (m) — Station × Hour",
                        scene=dict(
                            xaxis_title="Station",
                            yaxis_title="Hour",
                            zaxis_title="Residual Head (m)",
                        ),
                        height=600,
                    )
                    st.plotly_chart(_fig_3d_rh, use_container_width=True)

                # ── 24-Hour Aggregated Analysis ───────────────────────────────
                st.subheader("24-Hour Aggregated Analysis")

                # Total 24h cost by station (Power + DRA stacked bar)
                _agg_rows = []
                for _s in stations_base:
                    _k = str(_s.get("name", "")).strip().lower().replace(" ", "_")
                    _pc24 = sum(float(_r["result"].get(f"power_cost_{_k}", 0) or 0) for _r in reports)
                    _dc24 = sum(float(_r["result"].get(f"dra_cost_{_k}", 0) or 0) for _r in reports)
                    if _pc24 > 0 or _dc24 > 0:
                        _agg_rows.append({
                            "Station": _s.get("name", _k),
                            "Power Cost (INR)": _pc24,
                            "DRA Cost (INR)": _dc24,
                            "Total": _pc24 + _dc24,
                        })
                if _agg_rows:
                    _df_agg = pd.DataFrame(_agg_rows).sort_values("Total", ascending=True)
                    _fig_agg_cost = go.Figure()
                    _fig_agg_cost.add_trace(go.Bar(
                        y=_df_agg["Station"], x=_df_agg["Power Cost (INR)"],
                        name="Power Cost (INR)", orientation="h",
                        marker_color="#1967d2",
                    ))
                    _fig_agg_cost.add_trace(go.Bar(
                        y=_df_agg["Station"], x=_df_agg["DRA Cost (INR)"],
                        name="DRA Cost (INR)", orientation="h",
                        marker_color="#f59e0b",
                    ))
                    _fig_agg_cost.update_layout(
                        barmode="stack",
                        title="Total 24-Hour Cost by Station (Power + DRA)",
                        xaxis_title="Total Cost (INR)",
                        yaxis_title="Station",
                        height=max(300, len(_agg_rows) * 50 + 120),
                    )
                    st.plotly_chart(_fig_agg_cost, use_container_width=True)

                # Total DRA consumed (ppm-hours) by station
                _dra_agg_rows = []
                for _s in stations_base:
                    _k = str(_s.get("name", "")).strip().lower().replace(" ", "_")
                    _dra24 = sum(float(_r["result"].get(f"dra_ppm_{_k}", 0) or 0) for _r in reports)
                    if _dra24 > 0:
                        _dra_agg_rows.append({"Station": _s.get("name", _k), "DRA ppm-hours": _dra24})
                if _dra_agg_rows:
                    _df_dra_agg = pd.DataFrame(_dra_agg_rows).sort_values("DRA ppm-hours", ascending=True)
                    _fig_dra_agg = go.Figure(go.Bar(
                        y=_df_dra_agg["Station"], x=_df_dra_agg["DRA ppm-hours"],
                        orientation="h", marker_color="#0ea5e9",
                    ))
                    _fig_dra_agg.update_layout(
                        title="Total DRA Consumed — 24 Hour Cumulative (ppm·hr by Station)",
                        xaxis_title="DRA (ppm-hours)",
                        yaxis_title="Station",
                        height=max(300, len(_dra_agg_rows) * 50 + 120),
                    )
                    st.plotly_chart(_fig_dra_agg, use_container_width=True)

                # Cost per unit volume (INR/m³) by station
                _cpv_rows = []
                for _s in stations_base:
                    _k = str(_s.get("name", "")).strip().lower().replace(" ", "_")
                    _tc24 = sum(
                        float(_r["result"].get(f"power_cost_{_k}", 0) or 0)
                        + float(_r["result"].get(f"dra_cost_{_k}", 0) or 0)
                        for _r in reports
                    )
                    _vol24 = sum(float(_r["result"].get("flow_m3hr", 0) or 0) for _r in reports)
                    if _tc24 > 0 and _vol24 > 0:
                        _cpv_rows.append({
                            "Station": _s.get("name", _k),
                            "Cost/m³ (INR)": _tc24 / _vol24,
                        })
                if _cpv_rows:
                    _df_cpv = pd.DataFrame(_cpv_rows).sort_values("Cost/m³ (INR)", ascending=True)
                    _fig_cpv = go.Figure(go.Bar(
                        y=_df_cpv["Station"], x=_df_cpv["Cost/m³ (INR)"],
                        orientation="h", marker_color="#8b5cf6",
                    ))
                    _fig_cpv.update_layout(
                        title="Cost per Unit Volume by Station (INR/m³)",
                        xaxis_title="Cost per m³ (INR)",
                        yaxis_title="Station",
                        height=max(300, len(_cpv_rows) * 50 + 120),
                    )
                    st.plotly_chart(_fig_cpv, use_container_width=True)
        ...
        with tab_downloads:
            ...
            def _generate_pdf_report(_rpts, _stns, _plan_df, _term, _f_mode, _dmv):
                ...
                try:
                    ...
                    # ── flow key helpers (fix: flow_m3hr not set; use pump_flow_{origin}) ──
                    _origin_sk = _rk(_pump_stns[0].get("name", "")) if _pump_stns else ""
                    def _get_flow(r):
                        v = _f(r["result"].get(f"pump_flow_{_origin_sk}"))
                        return v if v > 0 else _f(r["result"].get("flow_m3hr"))
                    ...
                    _flows = [_get_flow(r) for r in _rpts] if _rpts else []
                    ...
                    # ── pump efficiency (fix: use efficiency_{sk} not pump_eff_{sk}) ────
                    _eff_vals = []
                    for _r0 in (_rpts or []):
                        for _s0 in (_pump_stns or []):
                            _esk = _rk(_s0.get("name", ""))
                            _ev = _f(_r0["result"].get(f"efficiency_{_esk}",
                                     _r0["result"].get(f"pump_eff_{_esk}")))
                            if _ev > 0:
                                _eff_vals.append(_ev)
                    _avg_eff = sum(_eff_vals) / len(_eff_vals) if _eff_vals else 0.0

                    # ── SEC (Specific Energy Consumption) in INR/MT/km ────────────────
                    _km_main = sum(_f(s.get("L", 0)) for s in (_stns or []))
                    _km_loop = sum(_f((s.get("loopline") or {}).get("L", 0))
                                   for s in (_stns or []) if s.get("loopline"))
                    _total_km = max(_km_main + _km_loop, 1.0)
                    _avg_density = 850.0
                    _total_MT = 0.0
                    if _plan_df is not None and not _plan_df.empty:
                        _vol_col = next((c for c in _plan_df.columns
                                         if "vol" in c.lower()), None)
                        _den_col = next((c for c in _plan_df.columns
                                         if "dens" in c.lower()), None)
                        if _vol_col and _den_col:
                            try:
                                import pandas as _pd_sec
                                _total_MT = sum(
                                    float(_pd_sec.to_numeric(row[_vol_col], errors="coerce") or 0) *
                                    float(_pd_sec.to_numeric(row[_den_col], errors="coerce") or 850) / 1000
                                    for _, row in _plan_df.iterrows()
                                )
                            except Exception:
                                _total_MT = 0.0
                    if _total_MT <= 0.0:
                        try:
                            ...
                            _avg_density = float(_st_sec.session_state.get("laced_density_kgm3", 850.0))
                        except Exception:
                            _avg_density = 850.0
                        _total_MT = _total_vol * _avg_density / 1000.0
                    _total_power_cost = sum(
                        sum(_f(r["result"].get(f"power_cost_{_rk(s.get('name',''))}"))
                    ...
                    def _draw_table(pdf, headers, rows, col_widths, font_sz=8.5, align="C", page_orientation="P"):
                    ...
                    # info box — dynamic height to fit all KV rows
                    ...
                    _cover_kv = [
                        ...
                        ("Unit Pipeline Transportation Cost - Elec. only (INR/MT/km)", f"{_uptc_elec:.6f}"),
                        ("Unit Pipeline Transportation Cost - Total incl. DRA (INR/MT/km)", f"{_uptc_total:.6f}"),
                    ...
                    _summ_rows = [
                        ...
                        ["Total Volume Throughput", f"{_total_vol:,.1f}", "m3"],
                        ...
                        ["Unit Pipeline Transportation Cost (Elec. only)", f"{_uptc_elec:.6f}", "INR/MT/km"],
                        ["Unit Pipeline Transportation Cost (Total incl. DRA)", f"{_uptc_total:.6f}", "INR/MT/km"],
                    ...
                    _summary_paras = [
                        ...
                        f"flow mode. The system processed a total throughput of {_total_vol:,.1f} m3 "
                        ...
                        f"Unit Pipeline Transportation Cost: {_uptc_elec:.6f} INR/MT/km (electricity only), "
                    ...
                    # ════════════════════════════════════════════════════════════════════
                    # CHAPTER 2: OPTIMISED OPERATION PLAN
                    # ════════════════════════════════════════════════════════════════════
                    _pdf.add_page()
                    _pdf.chapter_title("Optimised Operation Plan")
                    _pdf.section_title("2.1  Operation Plan by Time Slab")
                    _pdf.body_text(
                        "The following plan shows each time slab during which pump speed, pump type "
                        "selection, and DRA injection dosage remain constant at a given station. "
                        "A new slab begins whenever any of these three parameters changes."
                    )
                    _pdf.ln(2)

                    # Build time-slab data per station
                    _op_plan_rows = []
                    for _op_ps in _pump_stns:
                        _op_sk = _rk(_op_ps.get("name", ""))
                        _op_stn_name = _san(str(_op_ps.get("name", _op_sk)))
                        _op_ptypes_all = {k: v for k, v in (_op_ps.get("pump_types") or {}).items()
                                          if isinstance(v, dict)}
                        _op_hours_sig = []
                        for _op_r in _rpts:
                            _op_res = _op_r["result"]
                            _op_spd = round(_f(_op_res.get(f"speed_{_op_sk}")), 0)
                            _op_dra = round(_f(_op_res.get(f"dra_ppm_{_op_sk}")), 1)
                            # Detect active pump types by type-specific speed keys
                            _op_active = {}
                            for _op_tk, _op_tv in _op_ptypes_all.items():
                                _op_tspd = _f(_op_res.get(f"speed_{_op_sk}_{_op_tk}"))
                                if _op_tspd > 0:
                                    _op_active[_op_tk] = round(_op_tspd, 0)
                            # fallback: no type-specific keys → use station-level speed
                            if not _op_active and _op_spd > 0:
                                _op_active = {"Pump": _op_spd}
                            _op_hours_sig.append((_op_r["time"], _op_spd, _op_dra,
                                                  tuple(sorted(_op_active.items()))))

                        # Group consecutive hours with same signature into slabs
                        _op_slabs = []
                        if _op_hours_sig:
                            _op_cur = list(_op_hours_sig[0])
                            _op_cur_start = _op_cur[0]
                            for _op_h in _op_hours_sig[1:]:
                                if (_op_h[1], _op_h[2], _op_h[3]) == (_op_cur[1], _op_cur[2], _op_cur[3]):
                                    _op_cur[0] = _op_h[0]  # extend end
                                else:
                                    _op_slabs.append((_op_cur_start, _op_cur[0],
                                                      _op_cur[1], _op_cur[2], _op_cur[3]))
                                    _op_cur_start = _op_h[0]
                                    _op_cur = list(_op_h)
                            _op_slabs.append((_op_cur_start, _op_cur[0],
                                              _op_cur[1], _op_cur[2], _op_cur[3]))

                        for _op_sl in _op_slabs:
                            _op_tstart, _op_tend, _op_spd2, _op_dra2, _op_types2 = _op_sl
                            _op_time_str = f"{_op_tstart:02d}:00-{_op_tend + 1:02d}:00"
                            # Build pump config string: Type@RPM+Type@RPM...
                            if _op_types2:
                                _op_pump_str = "+".join(
                                    f"{_san(_tk)}@{int(_ns)} rpm"
                                    for _tk, _ns in _op_types2)
                            else:
                                _op_pump_str = f"{int(_op_spd2)} rpm" if _op_spd2 > 0 else "Off"
                            _op_dra_str = (f"{_op_dra2:.0f} ppm" if _op_dra2 > 0 else "Zero")
                            _op_plan_rows.append([
                                _op_time_str,
                                _op_stn_name,
                                _op_pump_str,
                                _op_dra_str,
                    ...
                    _op_plan_rows.sort(key=lambda x: x[0])  # sort by time slab
                    if _op_plan_rows:
                        _op_hdrs = ["Time Slab", "Station", "Pump Configuration", "DRA Dosage"]
                        _draw_table(_pdf, _op_hdrs, _op_plan_rows, [32, 40, 82, 26], font_sz=9)
                    else:
                        _pdf.body_text("No hourly optimization data available.")
                    _pdf.ln(4)
                    ...
                    # ════════════════════════════════════════════════════════════════════
                    _pdf.add_page(orientation="L")   # landscape for wide table
                    ...
                    if _rpts:
                        ...
                        for _ps in _pump_stns:
                            ...
                            _sname2 = str(_ps.get("name", _rk2))[:8]
                        ...
                        for _r0 in _rpts:
                            ...
                            for _ck in _stn_col_keys2:
                                ...
                                _row0.append(f"{_f(_res0.get(f'motor_kw_{_ck}')):.0f}")
                                _row0.append(f"{_f(_res0.get(f'dra_ppm_{_ck}')):.1f}")
                                _eff_v = _f(_res0.get(f"efficiency_{_ck}",
                                            _res0.get(f"pump_eff_{_ck}")))
                                _row0.append(f"{_eff_v:.1f}" if _eff_v > 0 else "-")
                            _sched_rows2.append(_row0)

                        _base_w = [18, 22, 26]
                        _per_stn_w = [16, 16, 15, 15]
                        _sched_cw = _base_w + _per_stn_w * len(_pump_stns)
                        _total_w2 = sum(_sched_cw)
                        _ls_avail = 265.0
                        if _total_w2 > _ls_avail:
                            _sc2 = _ls_avail / _total_w2
                            _sched_cw = [w * _sc2 for w in _sched_cw]
                        _draw_table(_pdf, _sched_hdrs, _sched_rows2, _sched_cw, font_sz=7.0, page_orientation="L")
                        _pdf.ln(4)
                    ...
                    if _rpts and _costs:
                        ...
                        _sched_analysis = (
                            ...
                            f"of minimum cost). "
                            f"Flow rates varied by {_flow_range:,.0f} m3/hr across the schedule, "
                    ...
                    if _rpts and _costs:
                        ...
                        for _bar, _v in zip(_bars41, _costs):
                            _ax41.text(_bar.get_x() + _bar.get_width() / 2,
                                        _bar.get_height() + _peak_cost * 0.005,
                        ...
                        _ax41.set_ylim(0, _peak_cost * 1.35)
                        _fig41.tight_layout()
                    ...
                    if _rpts and _pump_stns:
                        ...
                        _stn_linestyles = ["solid", "dashed", "dotted", "dashdot",
                                            (0, (3, 1, 1, 1)), (0, (5, 1))]
                        _stn_markers = ["o", "s", "^", "D", "v", "P"]
                        for _si, _ps in enumerate(_pump_stns):
                            _sk = _rk(_ps.get("name", ""))
                            _ppms = [_f(r["result"].get(f"dra_ppm_{_sk}")) for r in _rpts]
                            _c61 = _stn_colors2[_si % len(_stn_colors2)]
                            _ls61 = _stn_linestyles[_si % len(_stn_linestyles)]
                            _mk61 = _stn_markers[_si % len(_stn_markers)]
                            _ax61.plot(_hour_labels, _ppms,
                                        marker=_mk61, linewidth=2, linestyle=_ls61,
                                        markersize=6, color=_c61,
                                        label=str(_ps.get("name", _sk)))
                    ...
                    if _rpts and _pump_stns:
                        ...
                        for _r0 in _rpts:
                            ...
                            for _ps in _pump_stns:
                                ...
                                _brow.append(f"{_f(_r0['result'].get(f'motor_kw_{_sk}')):.1f}")
                        ...
                        _bkw_base = [18, 20]
                        _bkw_per = [14, 16, 16, 13, 18]
                        _bkw_cw = _bkw_base + _bkw_per * len(_pump_stns)
                        if sum(_bkw_cw) > 265.0:
                            _bsc = 265.0 / sum(_bkw_cw)
                            _bkw_cw = [w * _bsc for w in _bkw_cw]
                        _draw_table(_pdf, _bkw_hdrs, _bkw_rows, _bkw_cw, font_sz=6.5, page_orientation="L")
                        _pdf.ln(3)

                        # ── 7.5  Hourly SEC table ──────────────────────────────────────
                        _pdf.add_page(orientation="L")
                        _pdf.section_title("7.5  Hourly Unit Pipeline Transportation Cost & SEC")
                        ...
                        _draw_table(_pdf, _sec_hdrs75, _sec_rows75,
                                     [18, 24, 24, 30, 20, 38, 32, 30], font_sz=8.0, page_orientation="L")
                        _pdf.ln(3)
                        ...
                        _all_pw = [_f(r["result"].get(f"motor_kw_{_rk(s.get('name',''))}"))
                                    for r in _rpts for s in _pump_stns
                                    if _f(r["result"].get(f"motor_kw_{_rk(s.get('name',''))}")) > 0]
                        _peak_pw = max((sum(_f(r["result"].get(f"motor_kw_{_rk(s.get('name',''))}"))
                    ...
                    if _rpts and _flows:
                        _max_fv = max(_flows) if _flows else 1.0
                        ...
                        _ax81.axhline(_avg_flow, color=_ORANGE, linewidth=1.2, linestyle="--",
                                       label=f"Average: {_avg_flow:,.1f} m3/hr")
                        if _dmv and _dmv > 0:
                            _target_hourly = _dmv / 24.0
                            _ax81.axhline(_target_hourly, color=_GREEN, linewidth=1.0,
                                           linestyle=":", label=f"Target: {_target_hourly:,.1f} m3/hr")
                        for _xi, (_hl2, _fv) in enumerate(zip(_hour_labels, _flows)):
                            _ax81.text(_xi, _fv + _max_fv * 0.02,
                                        f"{_fv:,.0f}", ha="center", va="bottom",
                                        fontsize=6.5, color="#333333")
                        _style_ax(_ax81, "Hourly Flow Rate (m3/hr)",
                                   "Hour of Day", "Flow Rate (m3/hr)")
                        ...
                        _ax82b = _ax82a.twinx()
                        _ax82a.plot(_hour_labels, _uptc_hourly82, color=_GREEN, linewidth=2,
                        ...
                        _ax82a.axhline(_avg_uptc_hr82, color=_GREEN, linewidth=1.2, linestyle="--",
                                       alpha=0.7)
                        _ax82b.plot(_hour_labels, _sec_kwh_hourly82, color=_ORANGE, linewidth=2,
                                    marker="s", markersize=5, zorder=3)
                        _ax82b.axhline(_avg_sec_kwh82, color=_ORANGE, linewidth=1.2, linestyle="--",
                                       alpha=0.7)
                        _style_ax(_ax82a, "Hourly Unit Pipeline Transportation Cost & SEC",
                                   "Hour of Day", "Unit Pipeline Transportation Cost (INR/MT/km)")
                    ...
                    # ════════════════════════════════════════════════════════════════════
                    _pdf.add_page()
                    _pdf.chapter_title("Optimizer Recommendations")
                    _rec_items = []
                    ...
                    if _uptc_elec > 0 or _sec_kwh > 0:
                        _rec_items.append(
                            f"The optimized schedule achieves a Unit Pipeline Transportation Cost "
