...
with st.expander("⬇️ Upload/Download Pipeline Network as JSON", expanded=False):
    ...
    if upload is not None:
        try:
            ...
            # Convert back to Node/Edge objects if needed
            def to_node(n):
                if isinstance(n, Node): return n
                return Node(**n)
            def to_edge(e):
                if isinstance(e, Edge): return e
                return Edge(**e)
            network["nodes"] = [to_node(n) for n in network["nodes"]]
            network["edges"] = [to_edge(e) for e in network["edges"]]
            st.session_state.network_data = network
            st.rerun()
        except Exception as ex:
            st.error(f"Error loading network: {ex}")
    st.download_button(
...
for node in st.session_state.network_data["nodes"]:
    ...
    with st.expander(f"Edit {node.label}", expanded=False):
        ...
        if is_pump:
            ...
            # (You can extend here for full pump curve input later!)
...
def calculate_heads(stations, node_id, downstream_head=0):
    ...
    if node['parent'] is not None:
        ...
        # Example: Swamee-Jain (for real use, swap with your production code)
        f = 0.019
        g = 9.81
        Q = flow / 3600.0  # m3/s
        A = 3.1416 * (D ** 2) / 4
