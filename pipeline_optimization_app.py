...
st.markdown("<h1>Mixed Integer Nonlinear Optimization of Pipeline Operations</h1>",
            unsafe_allow_html=True)
...
for idx, stn in enumerate(st.session_state.stations, start=1):
    with st.expander(f"Station {idx}", expanded=True):
        ...
        stn['t']     = st.number_input("Wall Thickness (m)", value=stn['t'],
                                       format="%.4f", step=1e-4, key=f"t{idx}")
        stn['SMYS']  = st.number_input("SMYS (psi)", value=stn['SMYS'],
                                       step=1000.0, key=f"SMYS{idx}")
        stn['rough'] = st.number_input("Pipe Roughness (m)",
                                       value=stn['rough'], format="%.5f",
                                       step=1e-5, key=f"rough{idx}")
...
if run:
    with st.spinner("Solving optimization..."):
        ...
        for idx, stn in enumerate(stations_data, start=1):
            if stn.get('is_pump'):
                dfh = st.session_state[f"head_data_{idx}"]
                dfe = st.session_state[f"eff_data_{idx}"]
                if len(dfh)<3 or len(dfe)<5:
                    st.error("Need ≥3 head & ≥5 eff points.")
                    st.stop()
                # fit
                Qh,Hh = dfh.iloc[:,0], dfh.iloc[:,1]
                c2,c1,c0 = np.polyfit(Qh,Hh,2)
                stn['A'],stn['B'],stn['C'] = c2,c1,c0
                Qe,Ee = dfe.iloc[:,0], dfe.iloc[:,1]
                coeff = np.polyfit(Qe,Ee,4)
                stn['P'],stn['Q'],stn['R'],stn['S'],stn['T'] = coeff
            # peaks
            pk = st.session_state[f"peak_data_{idx}"]
            peaks = []
            for _,r in pk.iterrows():
                loc,elev = float(r[0]), float(r[1])
                if not (0<=loc<=stn['L']) or elev<stn['elev']:
                    st.error("Invalid peak")
                    st.stop()
                peaks.append({'loc':loc,'elev':elev})
            stn['peaks'] = peaks
...
if 'res' in st.session_state:
    ...
    total_pumps  = sum(int(res[f"num_pumps_{s['name'].lower().replace(' ','_')}"]) 
                       for s in stations_data)
    speeds       = [res[f"speed_{s['name'].lower().replace(' ','_')}"] 
                    for s in stations_data]
    effs         = [res[f"efficiency_{s['name'].lower().replace(' ','_')}"] 
                    for s in stations_data]
    avg_speed    = np.mean([s for s in speeds if s])
    avg_eff      = np.mean([e for e in effs if e])
    ...
    # Summary
    if view == "Summary":
        st.markdown("<div class='section-title'>Optimization Results</div>",
                    unsafe_allow_html=True)
        ...
        st.download_button(
            "📥 CSV", df_sum.to_csv(index=False).encode(),
    ...
    # Cost Breakdown
        ...
        df_cost = pd.DataFrame({
            ...
            "Power+Fuel": [res[f"power_cost_{s['name'].lower().replace(' ','_')}"]
                           for s in stations_data],
            "DRA":       [res[f"dra_cost_{s['name'].lower().replace(' ','_')}"]
                           for s in stations_data]
        })
        fig = px.bar(
    ...
    # Performance
        ...
        perf_tab, head_tab, pHQ, pEQ, pPS, pPF = st.tabs([
            "Head Loss","Vel & Re","Pump H–Q","Pump η–Q",
            "Power vs Speed","Power vs Flow"
        ])

        with perf_tab:
            st.markdown("<div class='section-title'>Head Loss per Segment</div>",
                        unsafe_allow_html=True)
            df_h = pd.DataFrame({
                "Station":[s['name'] for s in stations_data],
                "Head Loss":[res[f"head_loss_{s['name'].lower().replace(' ','_')}"]
                            for s in stations_data]
            })
        ...
        with head_tab:
            st.markdown("<div class='section-title'>Velocity & Reynolds</div>",
                        unsafe_allow_html=True)
            df_v = pd.DataFrame({
                "Station":[s['name'] for s in stations_data],
                "Velocity (m/s)": [res[f"velocity_{s['name'].lower().replace(' ','_')}"]
                                   for s in stations_data],
                "Reynolds":     [res[f"reynolds_{s['name'].lower().replace(' ','_')}"]
                                   for s in stations_data]
            })
            st.dataframe(df_v.style.format({"Velocity (m/s)":"{:.2f}",
                                            "Reynolds":"{:.0f}"}))

        # Pump H–Q
        with pHQ:
            st.markdown("<div class='section-title'>Pump H–Q</div>",
                        unsafe_allow_html=True)
            for stn in stations_data:
                ...
                for rpm in np.arange(stn["MinRPM"], dol+1, 100):
                    fig.add_trace(go.Scatter(
                        x=flows,
                        y=(A*flows**2 + B*flows + C)*(rpm/dol)**2,
                        mode="lines", name=f"{rpm} rpm"
                    ))
                fig.update_layout(title=stn['name'],
                                  xaxis_title="Flow", yaxis_title="Head")
                st.plotly_chart(fig, use_container_width=True)

        # Pump η–Q
        with pEQ:
            st.markdown("<div class='section-title'>Pump η–Q</div>",
                        unsafe_allow_html=True)
            for stn in stations_data:
                if not stn['is_pump']: continue
                key = stn['name'].lower().replace(' ','_')
                ...
                for rpm in np.arange(stn["MinRPM"], dol+1, 100):
                    feq = FLOW * dol / rpm
                    η = (P*feq**4 + Q*feq**3 + R*feq**2 + S*feq + T)/100
                    fig.add_trace(go.Scatter(
                        x=flows, y=[η]*len(flows),
                        mode="lines", name=f"{rpm} rpm"
                    ))
                fig.update_layout(title=stn['name'],
                                  xaxis_title="Flow", yaxis_title="Efficiency")
                st.plotly_chart(fig, use_container_width=True)

        # Power vs Speed
        with pPS:
            st.markdown("<div class='section-title'>Power vs Speed</div>",
                        unsafe_allow_html=True)
            for stn in stations_data:
                if not stn['is_pump']: continue
                key = stn['name'].lower().replace(' ','_')
                ...
                for rpm in rpms:
                    ...
                    feq = FLOW * dol / rpm
                    η   = (stn['P']*feq**4 + stn['Q']*feq**3 + stn['R']*feq**2 +
                           stn['S']*feq + stn['T'])/100.0
                    pwr = (stn['rho']*FLOW*9.81*H*num)/(3600*1000*η*0.95)
                    powers.append(pwr)
                fig = go.Figure(go.Scatter(x=rpms, y=powers, mode="lines+markers"))
                fig.update_layout(title=stn['name'],
                                  xaxis_title="RPM", yaxis_title="Power (kW)")
                st.plotly_chart(fig, use_container_width=True)

        # Power vs Flow
        with pPF:
            st.markdown("<div class='section-title'>Power vs Flow</div>",
                        unsafe_allow_html=True)
            for stn in stations_data:
                if not stn['is_pump']: continue
                key = stn['name'].lower().replace(' ','_')
                ...
                fig.update_layout(title=stn['name'],
                                  xaxis_title="Flow", yaxis_title="Power (kW)")
                st.plotly_chart(fig, use_container_width=True)

    # System Curves
    elif view == "System Curves":
        st.markdown("<div class='section-title'>System Head Curves</div>",
                    unsafe_allow_html=True)
        for stn in stations_data:
            if not stn['is_pump']: continue
            key    = stn['name'].lower().replace(' ','_')
            ...
            for dra in np.arange(0, stn['max_dr']+1, 5):
                ...
                f  = np.where(Re>0,
                              0.25/(np.log10(rough/d_i/3.7 + 5.74/Re**0.9)**2),
                              0)
                DH = f*((Lseg*1000)/d_i)*(v**2/(2*9.81))*(1-dra/100)
                curves.append(pd.DataFrame({
                    "Flow":np.linspace(0,FLOW,101),
                    "SDH": elev_i+DH,
                    "DR": dra
                }))
    ...
    # Pump-System Interaction
        ...
        st.markdown("<div class='section-title'>Pump vs System Interaction</div>",
                    unsafe_allow_html=True)
        for stn in stations_data:
            if not stn['is_pump']: continue
            key = stn['name'].lower().replace(' ','_')
            A,B,C = stn['A'],stn['B'],stn['C']
            min_r  = res[f"min_rpm_{key}"]
            dol    = res[f"dol_{key}"]
            num    = res[f"num_pumps_{key}"]
            kv     = stn['KV']; rough = stn['rough']
            Lseg   = stn['L']; elev = stn['elev']
            ...
            for dra in np.arange(0, stn['max_dr']+1, 5):
                ...
                f  = np.where(Re>0,
                              0.25/(np.log10(rough/(stn['D']-2*stn['t'])/3.7 + 5.74/Re**0.9)**2),
                              0)
                DH = f*((Lseg*1000)/(stn['D']-2*stn['t']))*(v**2/(2*9.81))*(1-dra/100)
                fig_int.add_trace(go.Scatter(
                    x=flows, y=elev+DH,
                    mode="lines", name=f"Sys {dra}% DRA"
                ))
            ...
            for rpm in np.arange(min_r, dol+1, 100):
                ...
                fig_int.add_trace(go.Scatter(
                    x=flows, y=Hp,
                    mode="lines", name=f"Pump {rpm} rpm"
                ))
    ...
    # Cost Landscape
        ...
        st.markdown("<div class='section-title'>Cost Landscape (RPM vs DRA)</div>",
                    unsafe_allow_html=True)
        for stn in stations_data:
            if not stn['is_pump']: continue
            key   = stn['name'].lower().replace(' ','_')
