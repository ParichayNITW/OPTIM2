...
if run:
    with st.spinner("Solving pipeline optimization..."):
        stations = ["Vadinar","Jamnagar","Rajkot","Surendranagar", terminal_name]
        terminal = {"name": terminal_name, "elevation": terminal_elevation, "residual_head": required_residual_head}
        res = solve_pipeline(stations, terminal, FLOW, KV, rho, RateDRA, Price_HSD)
    params = {
    ...
    with tab1:
        ...
        try:
            ...
            for k, v in {
                ...
                'Terminal Station Name': terminal_name,
                'Terminal Elevation (m)': terminal_elevation,
                'Required Residual Head (m)': required_residual_head
            }.items():
            ...
            add_chart(px.bar(
                ...
                x='Station', y='Amount', color='Type', barmode='group', title='Cost Breakdown per Station'
            ))
            add_chart(px.bar(
                pd.DataFrame({
                    'Station': stations,
