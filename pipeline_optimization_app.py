...
if run:
    ...
    with tab6:
        ...
        st.info("Select any combination of 3 axes and one color-dimension for 3D visualization. If X and Y form a rectangular grid, you'll see a true 3D surface; otherwise, a 3D scatter. Data is generated for the first pump station. Optimization must be run at least once.")
    
        # Axis and color selector widgets - always visible
        variable_options = {
        ...
        # Current fingerprint: hash of all current optimization inputs
        ...
        last_stations = st.session_state.get("last_stations_data")
        last_term = st.session_state.get("last_term_data")
        last_fp = st.session_state.get("last_input_fingerprint")
    
        # If optimization NEVER run, warn & exit
        if not last_res or not last_stations or not last_term:
            st.warning("Please run optimization at least once to enable 3D analysis.")
        else:
            # If fingerprint changed, warn but keep showing last results
            if last_fp != current_fingerprint:
                st.warning("Inputs have changed since last optimization. Plots below show results for the previous run. Click 'Run Optimization' to update.")
    
            res = last_res
            stations_data = last_stations
            term_data = last_term
    
            # First pump station index
            ...
            if pump_idx is None:
                ...
                grid_defaults = {
                    "TDH": [None], "Residual Head": [None], "Pump Efficiency": [None], "Power Cost": [None], "DRA Cost": [None]
                }
                X_vals = grid_sizes.get(x_var, grid_defaults.get(x_var, [0]))
                Y_vals = grid_sizes.get(y_var, grid_defaults.get(y_var, [0]))
                is_surface = (len(X_vals) > 1 and len(Y_vals) > 1 and x_var != y_var)
    
                # Data lists for plot
                Z_axis_list, C_list, X_list, Y_list = [], [], [], []
    
                # Meshgrid for 3D plot (Surface or Scatter)
                for xi in X_vals:
                    for yi in Y_vals:
                        # Prepare parameter for this mesh point
                        param = {
                            "No. of Pumps": int(xi if x_var == "No. of Pumps" else (yi if y_var == "No. of Pumps" else stn.get('max_pumps', 1))),
                            "%DR": float(xi if x_var == "%DR" else (yi if y_var == "%DR" else 0)),
                            "Flow": float(xi if x_var == "Flow" else (yi if y_var == "Flow" else st.session_state.get('FLOW', 1000.0))),
                            "Pump Speed": float(xi if x_var == "Pump Speed" else (yi if y_var == "Pump Speed" else N_max))
                        }
                        # Fill other params for safety
                        for k in ["No. of Pumps", "%DR", "Flow", "Pump Speed"]:
                            if k not in param:
                                param[k] = grid_sizes[k][0]
                        N_base = N_max
                        Q_adj = param["Flow"] * N_base / param["Pump Speed"] if param["Pump Speed"] > 0 else 0
                        A = res.get(f"coef_A_{key}", 0); B = res.get(f"coef_B_{key}", 0); Cc = res.get(f"coef_C_{key}", 0)
                        P = stn.get('P', 0); Qc = stn.get('Q', 0); R = stn.get('R', 0); S = stn.get('S', 0); T = stn.get('T', 0)
                        ...
                        # For surface mode, only store one Z/C per XY pair
                        Z_axis_list.append(computed_vars[z_var])
                        C_list.append(computed_vars[c_var])
                        X_list.append(xi)
                        Y_list.append(yi)
    
                # Surface plot if mesh, else 3D scatter
                if is_surface:
                    ...
                    fig = go.Figure(data=[go.Scatter3d(
                        x=X_list, y=Y_list, z=Z_axis_list,
                        mode='markers',
                        marker=dict(
                            size=4,
                            color=C_list,
