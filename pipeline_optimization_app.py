...
# --------- LINEFILL ENTRY (Viscosity & Density) ----------
with st.sidebar:
    st.markdown("#### Linefill Mapping (Viscosity & Density per segment)")
    num_segs = len(st.session_state['stations'])
    if 'linefill_df' not in st.session_state or len(st.session_state['linefill_df']) != num_segs:
...
for idx, stn in enumerate(st.session_state.stations, start=1):
    with st.expander(f"Station {idx}: {stn['name']}", expanded=False):
        ...
        # Tabs for advanced per-station inputs
...
if run:
    with st.spinner("Solving optimization..."):
        ...
        # --- Use linefill for KV and rho per segment
