...
def test_long_floor_respected_when_queue_already_treated():
    queue = [(200.0, 2.0)]
    stn_data = {
        "d_inner": 0.7,
        "idx": 0,
    ...
    pumped, _queue_after, _inj_ppm_main, floor_requires_injection = _update_mainline_dra(
        queue,
        stn_data,
        opt,
        segment_length=10.0,
    ...
    pumped_length = sum(float(length) for length, _ppm in pumped)
    assert math.isclose(pumped_length, 10.0, rel_tol=0, abs_tol=1e-9)
    assert not floor_requires_injection, "Existing long treated queue should satisfy the floor without extra injection"
